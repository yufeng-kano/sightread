...
Key: `(user_id, sha256, model, connection_id, connection_base_url, profile, profile_version, pages_spec, prompt_sha256, PIPELINE_VERSION)` — `connection_id` because the same model id on two different endpoints is not the same model, and `connection_base_url` (the endpoint snapshot taken at enqueue) because *editing* a connection's URL repoints what that id means: results parsed through the old endpoint must not answer uploads aimed at the new one. A key rotation alone does not invalidate the cache — same endpoint, same model, same output.
