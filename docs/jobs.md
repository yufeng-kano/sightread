...
- **A retry must not buy a second parse.** The cache answers with *finished* work only, which leaves one gap: a browser cannot tell a lost response from a lost request, so it retries an upload that may already have landed, and the same bytes become a second job that bills the user's key twice for one document. `submit_parse(reuse_in_flight=True)` — asked for by the web library and by nothing else — answers such a retry with the job already queued or running for that exact key instead of starting another. `/v1/parse` does not do this: an API client that posts the same file twice means it, and has `force` besides.
