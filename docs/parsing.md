...
- Every response's `usage` object is written to `usage_log` per call. OpenRouter always includes tokens + the actual `cost`; a generic OpenAI endpoint reports tokens only, so `cost` records as 0. Never maintain a local price table.
